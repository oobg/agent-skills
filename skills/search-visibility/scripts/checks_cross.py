...
def audit_cross(pages, sitemap_locs):
    ...
    out = {"checks": []}
    # 사이트맵 포함 대조는 1장으로도 계산된다. 2장 게이트에 묶으면 관측하고도 침묵한다.
    single = [p for p in pages if p.get("status") == 200]
    if sitemap_locs and single:
        indexed = {u.rstrip("/") for u in sitemap_locs}
        missing = [p["url"] for p in single if p["url"].rstrip("/") not in indexed
                   and (p.get("final_url") or "").rstrip("/") not in indexed]
        out["checks"].append((
    ...
    seen = unique
    if len(seen) < 2:
        # 미실행을 침묵으로 두면 "문제 없음"으로 읽힌다.
        out["checks"].append(("CHECK", "페이지 간 대조",
                              "미실행 — 1장만 관측했다. 제목 중복과 엔티티 표기는 --pages로 여러 장을 봐야 한다"))
        return out
