...
에이전트 스킬입니다. 코드 변경만으로 완료하지 않고, 크롤러가 읽은 결과와 재측정 일정까지를
...
- 작업 후 효과를 숫자로 확인하는 측정 루프를 설정할 때
...
핵심 검사 중 하나는 **구조화 데이터 대조**입니다. JSON-LD가 선언한 문답이 가시 텍스트에
...
- [`references/citation-competition.md`](references/citation-competition.md): 인용 경쟁 판독의
...
- [`references/ontology-boost.md`](references/ontology-boost.md): 온톨로지가 있을 때만 여는
  선택 모듈이며 질문 목록, 1차 소스 정의, 서비스명 표기와 과거 결정을 회수합니다.
