...
- subject 설명은 명사로 끝낸다. 동작은 `수정`, `추가`, `분리`, `제거`처럼 명사로 바꿔
  적고 `~하다`, `~한다`, `~했다`, `~함`, `~하기` 같은 용언 종결을 쓰지 않는다.
  - `fix(auth): 토큰 갱신 실패를 수정한다` → `fix(auth): 토큰 갱신 실패 수정`
  - `refactor(parser): 중복 분기를 제거함` → `refactor(parser): 중복 분기 제거`
  - `revert: 캐시 무효화 로직을 되돌린다` → `revert: 캐시 무효화 로직 복구`
- 이 종결 제한은 헤더 설명에만 적용한다. 본문 서술은 완결된 문장으로 쓴다.
