...
- 호출 한 번에 이미지 한 장을 만들며, 지정한 경로에서 PNG 시그니처와 파일 데이터가 확인된 경우에만 마지막 줄에
  `SAVED <절대경로>`를 출력합니다.
...
독립 이미지 N장이 필요하면 `--label`을 다르게 주어 N회 호출하며, 각 호출은 한 장만
요청합니다. 사용자가 N장을 명시했다면 다시 확인하지 않고 N회 진행합니다. 수량을 명시하지
않았다면 한 번 호출해 한 장만 만듭니다. 콜라주 한 장은 한 번 호출해 레이아웃을 프롬프트에
적습니다. 호출마다 PID와 난수로 만든 고유 ID가 붙어 파일명과 로그가 겹치지 않습니다.

스크립트는 호출별로 지정한 정확한 경로에서 PNG 시그니처와 파일 데이터가 확인된 경우만
성공으로 인정합니다. 플러그인이 다른 경로에 저장했거나 이 조건을 충족하지 않으면, 같은
폴더의 다른 이미지를 대신 고르지 않고 로그 경로와 함께 실패합니다.

## 요구 사항

- Codex CLI 설치와 `codex login`(ChatGPT 계정)
- `codex features list`에서 `image_generation`이 `stable true`
- Bash 3.2 이상. macOS 기본 `/bin/bash`(3.2)와 호환됩니다.
