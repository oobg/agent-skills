...
그 어체로 끝나지 않는 문장을 잡는다. 명사형을 목표로 삼을 수도 있다. 다만 명사
종결("저장 완료")과 음슴 종결은 안전하게 자동 분류하기 어려워, 두 어체에서는
해요체·합니다체·한다체처럼 명확한 혼용만 잡는다. 코드블록, 헤딩, 어미가 또렷하지
않은 조각은 검사에서 빠진다.
...
TARGETS = {
    ...
    "명사형": "명사", "명사체": "명사", "명사 종결": "명사",
    "해요": "해요", "합니다": "합니다", "한다": "한다", "음슴": "음슴", "개조식": "음슴",
}
LABEL = {
    "해요": "해요체", "합니다": "합니다체", "한다": "한다체",
    "음슴": "음슴체", "명사": "명사형",
}
NOMINAL_YO_ENDINGS = {"필요", "중요", "개요", "주요"}
...
def classify(tok):
    ...
    for suf in ("습니까", "ㅂ니까", "입니까", "니까", "습니다",
        ...
        if tok.endswith(suf):
            return "합니다"
    if tok.endswith("요") and tok not in NOMINAL_YO_ENDINGS:
        return "해요"
    ...
    if tok.endswith("다"):
        return "한다"
    return None  # 음슴·명사 종결은 분류하지 않음(상호 오탐 방지)
